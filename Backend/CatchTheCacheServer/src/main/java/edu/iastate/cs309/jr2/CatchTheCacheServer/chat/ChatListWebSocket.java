...
@ServerEndpoint(value = "/chats/websocket", configurator = CustomConfigurator.class)
@Component
public class ChatListWebSocket {
	// Store all socket session and their corresponding id
		private static List<Session> sessionList = new ArrayList<>();

		private static final Logger logger = LoggerFactory.getLogger(ChatListWebSocket.class);
		...
		public static void broadcast(String message) throws IOException {
			for (Session session : sessionList) {
				logger.info("Broadcasting for chat list to session: " + session.getId());
